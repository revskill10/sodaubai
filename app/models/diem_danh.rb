    ...
    if lop_mon_hoc
      l = lop_mon_hoc.lop_mon_hoc_sinh_viens.where(ma_sinh_vien: ma_sinh_vien).first    
       if l
        ...
        l.so_vang_co_phep = lop_mon_hoc.lich_trinh_giang_days.where("loai is NULL or loai != 1").inject(0){|sum,x| sum + x.diem_danhs.where(ma_sinh_vien: ma_sinh_vien).sum(:so_tiet_vang, :conditions => {:phep => true}) }
        l.so_tiet_vang =  lop_mon_hoc.lich_trinh_giang_days.where("loai is NULL or loai != 1").inject(0){|sum,x| sum + x.diem_danhs.where(ma_sinh_vien: ma_sinh_vien).sum(:so_tiet_vang) }
        l.diem_chuyen_can = l.diemcc
