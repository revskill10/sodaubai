class LopMonHoc < ActiveRecord::Base
  ...
  has_many :tkb_giang_viens, :foreign_key => 'ma_lop', :primary_key => 'ma_lop', :dependent => :destroy do 
    def with_giang_vien(ma_giang_vien)
      where("tkb_giang_viens.ma_giang_vien" => ma_giang_vien)
    end
  end
