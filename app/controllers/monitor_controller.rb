...
class MonitorController < ActionController::Base
  ...
  def load_phongs
    ...
    @lichs = LichTrinhGiangDay.where("ngay_day > timestamp ? and ngay_day < timestamp ? and loai is null", dt, DateTime.now).order('ngay_day asc, created_at asc')
    @lichs2 = LichTrinhGiangDay.where("ngay_day > timestamp ? and ngay_day < timestamp ? and loai is null", dt, DateTime.now).order('ngay_day asc, created_at asc').map {|l| "#{l.lop_mon_hoc.ma_lop}_#{l.lop_mon_hoc.ma_mon_hoc}"}
